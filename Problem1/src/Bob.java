...
public class Bob {
	...
	public byte[] Step2(byte[] step1package) throws IllegalStateException, IllegalBlockSizeException, BadPaddingException, NoSuchAlgorithmException {
		...
		{	
			...
			byte[] randomPlusOne = CommonFunctions.incrementByValue(step1package, (byte)1);
			...
			byte[] step2package = new byte[step1package.length + xoredValue.length];
			System.arraycopy(step1package, 0, step2package, 0, step1package.length);
			System.arraycopy(xoredValue, 0, step2package, step1package.length, xoredValue.length);
